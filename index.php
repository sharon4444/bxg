<?php
    ...
    if(array_key_exists("PATH_INFO", $_SERVER)){
        $path = $_SERVER["PATH_INFO"];//获取路径信息
    }
    
    $path = substr($path, 1, strlen($path));//php字符串截取，从第一个开始，截取到最后，把前面的/去掉
    $path = explode("/", $path);//php分割字符串成数组
    ...
    $realPath = "views/".$directoryName."/".$filename.".html";//返回
